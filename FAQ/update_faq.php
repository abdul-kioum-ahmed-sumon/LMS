...
<body class="container1">
    ...
    <main class="mt-5 pt-3" style="box-sizing:border-box; padding: 20px">
        ...
        <!-- Input fields for editing question and answer -->
        <form method="POST" action="#" class="mt-4">
            <label for="edit_question">Question:</label><br>
            <input type="text" id="edit_question" name="edit_question" required><br><br>
            <label for="edit_answer">Answer:</label><br>
            <textarea id="edit_answer" name="edit_answer" rows="4" required></textarea><br><br>
            <!-- Hidden input field to store FAQ ID -->
            <input type="hidden" id="faq_id" name="faq_id">
            <!-- Submit button for editing FAQ -->
            <input type="submit" name="submit" value="Edit FAQ">
        </form>
        <?php
        // Check if the edit form is submitted
        if (isset($_POST["submit"])) {
            // Retrieve edited question and answer from the form
            $question = $_POST['edit_question'];
            $answer = $_POST['edit_answer'];
            $id = $_GET['id']; // Retrieve FAQ ID from the URL parameter

            // Check if question and answer are not empty
            if (!empty($question) && !empty($answer)) {
                // Construct SQL query to update FAQ
                $temp =  "UPDATE faq
            SET question = '$question', answer = '$answer'
            WHERE id = $id";
                mysqli_query($con, $temp);
                // Redirect to the page displaying all FAQs
